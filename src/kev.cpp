...
json CKev::build_request(const std::string& state) {
    ...
    {"questions", {
        {"direction", {
            ...
            {"criteria", {{"up", nullptr}, {"down", nullptr}, {"left", nullptr}, {"right", nullptr}}}
        }}
    }}
    };
}
...
void CKev::worker_loop() {
    while (true) {
        ...
        cpr::Response r = cpr::Post(cpr::Url{ endpoint }, cpr::Body{ data.dump() }, cpr::Header{{"content-type", "application/json"}});
